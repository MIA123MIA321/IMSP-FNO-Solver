...
def train(x_train,y_train,x_test,y_test,device,model_pth='',name='',
    ...
    SEED_SET(1234)
    if model_pth=='':
        model = FNO2d_stack(FNO_ARG[0],FNO_ARG[1],FNO_ARG[2]).to(device)
    else:
...
if __name__ == '__main__':
    ...
    q_train, wave_train, u_train = load_data(train_filename, device = device,
                    NS_return = NS_return, Usage = 'Train1')
    q_test, wave_test, u_test = load_data(test_filename, device = device,
                        NS_return = NS_return, Usage = 'Train1')
...
#       start_lr=1e-4,end_lr=1e-5,epochs=200,early_stop=False,loss_level=2)
    ...
    # nohup python -u src/train1.py >> .train.log 2>&1 &
    
    
    
    train(x_train,y_train,x_test,y_test,device,
      '', NET_name+'_5',FNO_ARG=FNO_ARG,
      start_lr=2e-3,end_lr=1e-5,epochs=500,early_stop=False,loss_level=1)